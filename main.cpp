    ...
    for (int y=0; y<h; y++){
        for (int x=0; x<w; x++) {
            for( unsigned int s = 0 ; s < nsamples ; ++s ) {
                ...
                Vec3 color = scenes[selected_scene].rayTrace( Ray(pos , dir), camera.getNearPlane(), camera.getFarPlane() );
                image[x + y*w] += color;
            }
...
int main (int argc, char ** argv) {
    ...
    scenes[4].setup_cornell_mesh();
    scenes[5].setup_cornell_box_focal();
    scenes[6].setup_default_cornell_box();
    scenes[7].setup_cornell_finale();
